...
for tab in ("HOME", "SCENES", "STUDIO", "LIBRARY", "SETTINGS"):
...
check("placeholder engagement layer removed", not (ROOT / "app/src/main/java/com/unictoai/unictoos/domain/EngagementModels.kt").exists() and not (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/EngageScreen.kt").exists())
check("unrelated monetization layer removed", not (ROOT / "app/src/main/java/com/unictoai/unictoos/monetization/AdsPolicy.kt").exists() and "SponsorBanner" not in UI)
check("inactive integration stubs removed", not (ROOT / "app/src/main/java/com/unictoai/unictoos/integrations/PlatformIntegration.kt").exists() and not (ROOT / "app/src/main/java/com/unictoai/unictoos/integrations/DefaultPlatformIntegrationRegistry.kt").exists())
...
check("multistream callbacks aggregate all active slots", "containsAll(activeSlots)" in ADAPTER and "SlotConnectChecker" in ADAPTER and "Array<ConnectChecker>" in ADAPTER)
check("multistream reconnect clears partial slots", "activeSlots.isNotEmpty()" in ADAPTER and "if (isStreaming || synchronized(trackerLock)" in ADAPTER)
check("adapter validates endpoints before mutating slots", "normalizedEndpoints.all(StreamEndpointPolicy::isSupported)" in ADAPTER)
...
check("queued stream start is preserved", "CaptureStartPolicy.canQueue" in SERVICE and "pendingStart = request" in SERVICE)
check("capture preparation failures clear queued start", "private fun failCapturePreparation" in SERVICE and "pendingStart = null" in SERVICE and "Previous capture resources are not fully released" in SERVICE)
check("recording callback states are explicit", "RecordController.Status.STOPPED" in SERVICE and "RecordController.Status.PAUSED" in SERVICE and "RecordingState.STOPPING" in SERVICE)
...
SCENE_STORE = (ROOT / "app/src/main/java/com/unictoai/unictoos/data/SceneStore.kt").read_text()
EXPORTER = (ROOT / "app/src/main/java/com/unictoai/unictoos/data/ConfigExporter.kt").read_text()
check("scene orientation reaches encoder", "forAspectRatio" in QUALITY and "qualityForScene" in SERVICE and "EXTRA_SCENE_JSON" in MAIN)
check("truthful Studio network readiness", "NET_CAPABILITY_INTERNET" in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/StudioScreen.kt").read_text())
check("preview detach releases outstanding surface", "val detachedSurface = lastSurface" in PREVIEW_SURFACE if "PREVIEW_SURFACE" in globals() else "val detachedSurface = lastSurface" in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/PreviewSurfaceView.kt").read_text())
check("bounded scene persistence", "MAX_SCENES = 64" in SCENE_STORE and "MAX_SOURCES_PER_SCENE = 32" in SCENE_STORE and "minOf(length(), MAX_SCENES)" in SCENE_STORE)
check("valid JSON control escaping", "character.code < 0x20" in EXPORTER and "\\\\u" in EXPORTER)
...
check("SRT transport contract", "MultiType.SRT" in ADAPTER and "srt://" in ENDPOINT_POLICY and "srt://" in (ROOT / "app/src/main/java/com/unictoai/unictoos/StudioViewModel.kt").read_text())
check("strict complete endpoint validation", "URI(value)" in ENDPOINT_POLICY and "uri.host" in ENDPOINT_POLICY and "isSupported(value)" in (ROOT / "app/src/main/java/com/unictoai/unictoos/streaming/StreamPreflight.kt").read_text())
check("SRT destination needs no separate key", "startsWith(\"srt://\", ignoreCase = true) || streamKey.isNotBlank()" in (ROOT / "app/src/main/java/com/unictoai/unictoos/StudioViewModel.kt").read_text() and "destinationRequiresKeyForRtmpButNotForCompleteSrtListener" in (ROOT / "app/src/test/java/com/unictoai/unictoos/domain/StudioViewModelTest.kt").read_text())
...
check("focused bottom navigation", "GlassyBottomBar" in APP and "listOf(AppTab.HOME, AppTab.STUDIO, AppTab.LIBRARY)" in APP)
check("focused workspace menu", "AppTab.SCENES" in APP and "AppTab.SETTINGS" in APP and "AppTab.ENGAGEMENT" not in APP and "AppTab.MORE" not in APP)
HOME = (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/HomeScreen.kt").read_text()
check("Go Live is discoverable", 'STUDIO("Go Live")' in APP and "selectedTab = AppTab.STUDIO" in APP and 'else "Go Live"' in HOME)
check("first-run opens usable capture scene", 'mutableStateOf("main-camera")' in APP and 'Source("camera", "Face Cam", SourceType.CAMERA)' in (ROOT / "app/src/main/java/com/unictoai/unictoos/StudioViewModel.kt").read_text())
check("mixed scenes use shared camera-first policy", "CaptureModePolicy.forScene(scene)" in APP and "CaptureModePolicy.forScene(scene)" in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/StudioScreen.kt").read_text() and "mixedLegacyScenePrefersCamera" in (ROOT / "app/src/test/java/com/unictoai/unictoos/streaming/CaptureModePolicyTest.kt").read_text())
check("Go Live permission sequence", "RequestMultiplePermissions" in MAIN and "Manifest.permission.RECORD_AUDIO" in MAIN and "Manifest.permission.CAMERA" in MAIN and "launchProjection()" in MAIN)
CAPTURE_START_POLICY = (ROOT / "app/src/main/java/com/unictoai/unictoos/streaming/CaptureStartPolicy.kt").read_text()
CAPTURE_START_TEST = (ROOT / "app/src/test/java/com/unictoai/unictoos/streaming/CaptureStartPolicyTest.kt").read_text()
check("atomic capture and start handoff", "EXTRA_START_AFTER_PREPARE" in MAIN and "EXTRA_START_AFTER_PREPARE" in SERVICE and "queueCaptureStartIfRequested" in SERVICE and "ACTION_START" not in MAIN[MAIN.index("private val projectionLauncher"):MAIN.index("private val permissionLauncher")])
check("capture start policy is runtime-backed and tested", "CaptureStartPolicy.decide" in SERVICE and "CaptureStartPolicy.canQueue" in SERVICE and "queuedBeforePrepareWaitsThenStartsExactlyOnce" in CAPTURE_START_TEST and "STALE_REQUEST" in CAPTURE_START_POLICY)
check("capture preparation exceptions become visible errors", "projection_prepare_exception" in SERVICE and "camera_prepare_exception" in SERVICE and "failCapturePreparation" in SERVICE)
check("Go Live remains visible during setup", 'if (session.status == StreamStatus.LIVE) "Stop broadcast" else "Go Live"' in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/StudioScreen.kt").read_text())
check("permission flow does not skip empty destination", "pendingEndpoint.isBlank()" in MAIN and "Permissions ready. Add a streaming destination" in MAIN and "Add a streaming destination in Settings before going live" in MAIN)
check("Home readiness uses real permission state", "session.status != StreamStatus.ERROR" not in HOME and "NET_CAPABILITY_INTERNET" in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/components/SharedComponents.kt").read_text())
check("Home layout animation cost reduced", "animateContentSize" not in HOME and "AnimatedVisibility" not in HOME)
...
check("bounded active destination transport", "MAX_DESTINATIONS = 2" in ADAPTER and "activeSlots" in ADAPTER and "normalizedEndpoints" in ADAPTER)
check("active adapter teardown is retry-safe", "closed.set(false)" in ADAPTER and "firstFailure" in ADAPTER and "throw it" in ADAPTER)
...
check("v0.4.10 release metadata", 'versionName = "0.4.10"' in BUILD_GRADLE and "versionCode = 51" in BUILD_GRADLE and (ROOT / "VERSION").read_text().strip() == "0.4.10" and (ROOT / "RELEASE_NOTES_v0.4.10.md").exists() and (ROOT / "docs/V0.4_RESEARCH_AND_PRODUCT_PLAN.md").exists())
