...
check("empty capture source is blocked", 'else -> "none"' in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/UnictoosApp.kt").read_text() and 'captureMode = captureMode' in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/screens/StudioScreen.kt").read_text() and "Enable a camera or screen source" in (ROOT / "app/src/main/java/com/unictoai/unictoos/ui/MainActivity.kt").read_text() and "noCaptureSourceIsBlocking" in (ROOT / "app/src/test/java/com/unictoai/unictoos/streaming/GoLiveReadinessPolicyTest.kt").read_text())
...
check("first-run opens usable capture scene", 'mutableStateOf("main-camera")' in APP)
...
check("v0.4.7 release metadata", 'versionName = "0.4.7"' in BUILD_GRADLE and "versionCode = 48" in BUILD_GRADLE and (ROOT / "VERSION").read_text().strip() == "0.4.7" and (ROOT / "RELEASE_NOTES_v0.4.7.md").exists() and (ROOT / "docs/V0.4_RESEARCH_AND_PRODUCT_PLAN.md").exists())
